##############################################################
# ACPS Career Self-Efficacy Analysis- Community Lab School            
...
#          graders at Community Lab Middle School in the Fall of 2024
##############################################################
...
career_commlab <- subset(career_narm, school=="Community Lab School")
mean_commlab <- career_commlab %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                             obs=n())

##############################################################
# Community Lab Middle School- Career Self Efficacy by Ethnicity               
##############################################################
...
mean_Eth <- career_commlab %>% group_by(Ethnicity) %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                                                 obs=n())
...
ggplot(data=career_commlab, aes(y=total_score)) + 
  geom_point(data=career_commlab %>% dplyr::filter(Ethnicity=="WH"),
             aes(x=0, color=Ethnicity), position=position_jitter(0.1), alpha=1) +
...
ggsave("../plots/CommunityLab_CSE_eth_2024.pdf")
...
mean_commlab <- career_commlab %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                             obs=n())
career_shp <- subset(career_commlab, Starr_Hill_24=="Yes")
mean_shp <- career_shp %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                     obs=n())
career_avid <- subset(career_commlab, AVID=="Yes")
mean_avid <- career_avid %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                       obs=n())
career_el <- subset(career_commlab, EL=="Yes")
mean_el <- career_el %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                   obs=n())

career_sped <- subset(career_commlab, SPED=="Yes")
mean_sped <- career_sped %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                       obs=n())

# Plot Creation
#### Plot of Community Lab's Career Self Efficacy Scores for student groups of interest 

ggplot() + 
  geom_point(data=career_commlab, aes(y=total_score, x=-1), position=position_jitter(0.1), alpha=1, color="black") +
  ...
  scale_y_continuous(limits=c(50,105), minor_breaks=seq(50,105,5), breaks=seq(50,100,10)) +
  geom_segment(data= mean_commlab, aes(y=mean_score, yend=mean_score, x=-1.25, xend=-.75), linewidth=1, color ="black") +
  ...
  labs(x="Student Group", y="Career Self-Efficacy Score", 
       title="Community Lab Middle School Career Self Efficacy Scores by Student Group")

ggsave("../plots/CommunityLab_CSE_studentgrp_2024.pdf")