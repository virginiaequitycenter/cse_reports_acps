...
career <- read_excel("MS_Reports_Dataset_Career_SelfEfficacy_12_05_2024.xlsx")
...
colnames(career) = c("Gender", "Ethnicity", "class_year","school","AVID","EL","SPED","Starr_Hill_24", 
...
#career <- career %>% mutate(SE_subscore=rowMeans(career[,survey_seqs]) %>% round(digits=2))

#career <- career %>% mutate(OE_subscore=rowMeans(career[,survey_oeqs]) %>% round(digits=2))

#career <- career %>% mutate(total_score=rowMeans(career[,survey_qs]) %>% round(digits=2))
...
col_order <-  c("Gender","Ethnicity", "school","class_year", "AVID", "EL", "SPED", "Starr_Hill_24",
                "total_score","total_percentile","SE_subscore","SE_percentile","OE_subscore","OE_percentile",
...
##############################################################
# Write CSV to IVY Folder        
##############################################################

write_excel_csv(career, file = "Fall 2024 Career Self Efficacy Deidentified Clean.csv",na = "",
                append = FALSE, delim = ",")
