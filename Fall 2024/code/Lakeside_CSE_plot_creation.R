##############################################################
# ACPS Career Self-Efficacy Analysis- Lakeside Middle School            
...
#          graders at Lakeside Middle School in the Fall of 2024
##############################################################
...
mean_Eth <- career_lakeside %>% group_by(Ethnicity) %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                                                 obs=n())
...
ggplot(data=career_lakeside, aes(y=total_score)) + 
...
ggsave("../plots/Lakeside_CSE_eth_2024.pdf")
...
ggplot() + 
  geom_point(data=career_lakeside, aes(y=total_score, x=-1), position=position_jitter(0.1), alpha=1, color="black") +
...
ggsave("../plots/Lakeside_CSE_studentgrp_2024.pdf")