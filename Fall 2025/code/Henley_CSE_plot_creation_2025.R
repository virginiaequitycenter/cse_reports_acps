##############################################################
# ACPS Career Self-Efficacy Analysis- Henley Middle School Analysis - Main Graphs              
...
career_narm <- career %>% drop_na(total_score) %>% filter(year_completed==2025 & school_sy2526=="Joseph T. Henley Middle School")

##############################################################
# Henley CSE Wrangling (Find Group Level Means)              
##############################################################

mean_henley <- career_narm %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                         obs=n())
# by Ethnicity
...
ggsave("Fall 2025/plots/Henley_CSE_eth_2025.pdf")

##############################################################
# Henley CSE by Student Group Plot                    
##############################################################
...
ggsave("Fall 2025/plots/Henley_CSE_stugrp_2025.pdf")

##############################################################
# Henley CSE by Grade Plot                 
##############################################################
...
ggplot(data=career_narm, aes(y=total_score)) + 
  ...
  labs(x="Ethnicity", y="Career Self-Efficacy Score", 
       title="Henley Middle School Career Self Efficacy Scores by Grade")+
  geom_hline(yintercept=80,linetype="dashed")+
  scale_x_continuous(limits=c(-.3,.3))+
  scale_y_continuous(limits=c(50,110), minor_breaks=seq(50,110,5), breaks=seq(50,110,10))+
...
ggsave("Fall 2025/plots/Henley_CSE_grade_2025.pdf")
