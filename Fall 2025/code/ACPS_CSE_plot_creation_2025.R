##############################################################
# ACPS Career Self-Efficacy Analysis- All ACPS Schools Analysis - Main Graphs              
...
career_narm <- career %>% drop_na(total_score) %>% filter(year_completed==2025)

##############################################################
# ACPS CSE School Wrangling (Find Group Level Means)              
##############################################################

# by School
mean_ACPS <- career_narm %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                       obs=n())                                          
career_burley <- subset(career_narm, school_sy2526=="Jackson P. Burley Middle School")
mean_burley <- career_burley %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                           obs=n())
career_lab <- subset(career_narm, school_sy2526=="Community Lab School")
mean_lab <- career_lab %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                     obs=n())
career_henley <- subset(career_narm, school_sy2526=="Joseph T. Henley Middle School")
mean_henley <- career_henley %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                           obs=n())  
career_journey <- subset(career_narm, school_sy2526=="Journey Middle School")
mean_journey <- career_journey %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                             obs=n())
career_lakeside <- subset(career_narm, school_sy2526=="Lakeside Middle School")
mean_lakeside <- career_lakeside %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                               obs=n())
career_walton <- subset(career_narm, school_sy2526=="Leslie H. Walton Middle School")
...
ggsave("Fall 2025/plots/ACPS_CSE_byschool_2025.pdf")

##############################################################
# ACPS CSE by Ethnicity Plot                    
##############################################################

ggplot(data=career_narm, aes(y=total_score)) + 
  geom_point(aes(x=0, color=race_ethnicity), position=position_jitter(0.1), alpha=.5) +
  ...
  labs(x="Ethnicity", y="Career Self-Efficacy Score", 
       title="ACPS Schools Career Self Efficacy Scores by Ethnicity")+
  geom_hline(yintercept=80,linetype="dashed")+
...
ggsave("Fall 2025/plots/ACPS_CSE_eth_2025.pdf")

##############################################################
# ACPS CSE by Student Group Plot                    
##############################################################

ggplot() + 
  geom_point(data=career_narm, aes(y=total_score, x=-1), position=position_jitter(0.1), alpha=.5, color="black") +
  geom_point(data=career_shp, aes(y=total_score, x=0), position=position_jitter(0.1), alpha=.5, color="#8D029E") +
  geom_point(data=career_avid, aes(y=total_score, x=1), position=position_jitter(0.1), alpha=.5, color="#D9470C") +
  geom_point(data=career_el, aes(y=total_score, x=2), position=position_jitter(0.1), alpha=.5, color="#0C9ED9") +
  geom_point(data=career_dis, aes(y=total_score, x=3), position=position_jitter(0.1), alpha=.5, color="#139E02") +
  geom_point(data=career_firstgen, aes(y=total_score, x=4), position=position_jitter(0.1), alpha=.5, color="#F8BE3D") +
  scale_y_continuous(limits=c(50,110), minor_breaks=seq(50,110,5), breaks=seq(50,110,10)) +
  ...
  labs(x="Student Group", y="Career Self-Efficacy Score", 
       title="ACPS Career Self Efficacy Scores by Student Group")

ggsave("Fall 2025/plots/ACPS_CSE_stugrp_2025.pdf")

##############################################################
# ACPS CSE by Grade Plot                 
##############################################################

ggplot(data=career_narm, aes(y=total_score)) + 
  geom_point(aes(x=0, color=grade_at_time_of_survey), position=position_jitter(0.1), alpha=.5) +
  geom_hline(data= mean_grade, aes(yintercept = mean_score,col=grade_at_time_of_survey), linewidth=1) +
  labs(x="Ethnicity", y="Career Self-Efficacy Score", 
       title="ACPS School Career Self Efficacy Scores by Grade")+
  geom_hline(yintercept=80,linetype="dashed")+
  scale_x_continuous(limits=c(-.3,.3))+
  scale_y_continuous(limits=c(50,110), minor_breaks=seq(50,110,5), breaks=seq(50,110,10))+
...
ggsave("Fall 2025/plots/ACPS_CSE_grade_2025.pdf")
