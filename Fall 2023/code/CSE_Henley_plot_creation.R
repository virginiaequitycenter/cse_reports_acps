##############################################################
# ACPS Career Self-Efficacy Analysis- Henley Analysis- Main Graphs and Appendices               
# Authors: Asha Muralidharan     
# GitHub: asha-ec                              
# Last revised: 2024-02-01    
# Summary: A document that analyses the data from the Career Self-Efficacy 
#          survey administered to seventh and eighth graders at Henley Middle School
...
##############################################################
# Henley Middle School- Career Self Efficacy by Ethnicity               
##############################################################
...
mean_Eth <- career_henley %>% group_by(Ethnicity) %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                                                obs=n())
...
# Plot Creation
#### Plots Henley's Career self efficacy scores by ethnicity with mean value and individual scores

ggplot(data=career_henley, aes(y=total_score)) + 
  geom_point(data=career_henley %>% dplyr::filter(Ethnicity!="AS" & Ethnicity!="BL"),
    aes(x=0, color=Ethnicity), position=position_jitter(0.1), alpha=.7) +
  geom_hline(data= mean_Eth %>% dplyr::filter(Ethnicity!="AS" & Ethnicity!="BL"), aes(yintercept = mean_score,col=Ethnicity), linewidth=1) +
  labs(x="Ethnicity", y="Career Self-Efficacy Score", 
       title="Henley Middle School Career Self Efficacy Scores by Ethnicity")+
  geom_hline(yintercept=80,linetype="dashed")+
  scale_x_continuous(limits=c(-.3,.3))+
  scale_y_continuous(limits=c(50,105), minor_breaks=seq(50,105,5), breaks=seq(50,100,10))+
  scale_color_manual(values=c("#0C9ED9", "#139E02", "#F8BE3D"))+
...
ggsave("../plots/Henley Middle School Career Self Efficacy by Ethnicity.pdf")

##############################################################
# Henley Middle School- Career Self Efficacy by Student Groups                 
##############################################################
...
mean_henley <- career_henley %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                           obs=n())
career_shp <- subset(career_henley, SHP=="SHP")
mean_shp <- career_shp %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                     obs=n())
career_avid <- subset(career_henley, AVID=="AVID")
mean_avid <- career_avid %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                       obs=n())
career_el <- subset(career_henley, Current_EL=="EL")
mean_el <- career_el %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                   obs=n())
career_gg1 <- subset(career_henley, GG1=="GG1")
mean_gg1 <- career_gg1 %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                     obs=n())
career_sped <- subset(career_henley, SPED=="SPED")
mean_sped <- career_sped %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                       obs=n())

# Plot Creation
#### Plot of Henley's Career Self Efficacy Scores for student groups of interest 

ggplot() + 
  geom_point(data=career_henley, aes(y=total_score, x=-1), position=position_jitter(0.1), alpha=.7, color="black") +
  geom_point(data=career_avid, aes(y=total_score, x=0), position=position_jitter(0.1), alpha=.7, color="#D9470C") +
  ...
  scale_y_continuous(limits=c(50,105), minor_breaks=seq(50,105,5), breaks=seq(50,100,10)) +
  geom_segment(data= mean_henley, aes(y=mean_score, yend=mean_score, x=-1.25, xend=-.75), linewidth=1, color ="black") +
  geom_segment(data= mean_avid, aes(y=mean_score, yend=mean_score, x=-.25, xend=.25), linewidth=1, color ="#D9470C") +
  ...
  labs(x="Student Group", y="Career Self-Efficacy Score", 
       title="Henley Middle School Career Self Efficacy Scores by Student Group")

ggsave("../plots/Henley Middle School Career Self Efficacy by Student Group.pdf")


##############################################################
# Henley Self-Efficacy Subscores- by Ethnicity                          
##############################################################
...
career_henley <- subset(career_narm, School=="Joseph T. Henley Middle School")
mean_henley <- career_henley %>% summarise(mean_score=mean(SE_subscore, na.rm=TRUE),
                                           obs=n())  

mean_Eth <- career_henley %>% group_by(Ethnicity) %>% summarise(mean_score=mean(SE_subscore, na.rm=TRUE),
                                                                obs=n())
eth.labs <- c("Asian Students", "Black Students", "Hispanic Students", "Multiracial Students", "White Students")
names(eth.labs) <- c("AS", "BL", "HL", "MR", "WH")

# Plot Creation
#### Plots Henley's self efficacy subscores by ethnicity with mean value and individual scores

ggplot(data=career_henley, aes(y=SE_subscore)) + 
  geom_point(data=career_henley %>% dplyr::filter(Ethnicity!="AS" & Ethnicity!="BL"),
             aes(x=0, color=Ethnicity), position=position_jitter(0.1), alpha=.7) +
  geom_hline(data= mean_Eth %>% dplyr::filter(Ethnicity!="AS" & Ethnicity!="BL"), aes(yintercept = mean_score,col=Ethnicity), linewidth=1) +
  labs(x="Ethnicity", y="Self-Efficacy Subscore", 
       title="Henley Middle School Self Efficacy Subscores by Ethnicity")+
...
ggsave("../plots/Henley Middle School SE Subscore by Ethnicity.pdf")

##############################################################
# Henley Self-Efficacy Subscores- by Student Group                        
##############################################################
...
# Plot Creation
#### Plot of Henley's  Self Efficacy subscores for student groups of interest 

ggplot() + 
  geom_point(data=career_henley, aes(y=SE_subscore, x=-1), position=position_jitter(0.1), alpha=.7, color="black") +
  geom_point(data=career_avid, aes(y=SE_subscore, x=0), position=position_jitter(0.1), alpha=.7, color="#D9470C") +
  ...
  labs(x="Student Group", y="Self-Efficacy Subscore", 
       title="Henley Middle School Self Efficacy Subscores by Student Group")

ggsave("../plots/Henley Middle School SE Subscore by Student Group.pdf")

##############################################################
# Henley Outcome Expectancy Subscores- by Ethnicity                        
##############################################################
...
career_henley <- subset(career_narm, School=="Joseph T. Henley Middle School")
mean_henley <- career_henley %>% summarise(mean_score=mean(OE_subscore, na.rm=TRUE),
                                           obs=n())  

mean_Eth <- career_henley %>% group_by(Ethnicity) %>% summarise(mean_score=mean(OE_subscore, na.rm=TRUE),
                                                                obs=n())
eth.labs <- c("Asian Students", "Black Students", "Hispanic Students", "Multiracial Students", "White Students")
names(eth.labs) <- c("AS", "BL", "HL", "MR", "WH")
...
ggplot(data=career_henley, aes(y=OE_subscore)) + 
  geom_point(data=career_henley %>% dplyr::filter(Ethnicity!="AS" & Ethnicity!="BL"),
    aes(x=0, color=Ethnicity), position=position_jitter(0.1), alpha=.7) +
  geom_hline(data= mean_Eth %>% dplyr::filter(Ethnicity!="AS" & Ethnicity!="BL"), aes(yintercept = mean_score,col=Ethnicity), linewidth=1) +
  labs(x="Ethnicity", y="Outcome Expectancy Subscore", 
       title="Henley Middle School Outcome Expectancy Subscores by Ethnicity")+
...
ggsave("../plots/Henley Middle School OE Subscore by Ethnicity.pdf")


##############################################################
# Henley Outcome Expectancy Subscores- by Student Group                        
##############################################################
...
# Plot Creation
#### Plot of Henley's Outcome Expectancy subscores for student groups of interest 

ggplot() + 
  geom_point(data=career_henley, aes(y=OE_subscore, x=-1), position=position_jitter(0.1), alpha=.7, color="black") +
  geom_point(data=career_avid, aes(y=OE_subscore, x=0), position=position_jitter(0.1), alpha=.7, color="#D9470C") +
  ...
  labs(x="Student Group", y="Outcome Expectancy Subscore", 
       title="Henley Middle School Outcome Expectancy Subscores by Student Group")

ggsave("../plots/Henley Middle School OE Subscore by Student Group.pdf")
