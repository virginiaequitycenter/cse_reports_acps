...
##############################################################
# All ACPS- Career Self Efficacy by Ethnicity               
##############################################################
...
ggsave("../plots/ACPS Middle Schools Career Self Efficacy Scores by Ethnicity.pdf")

##############################################################
# All ACPS- Career Self Efficacy by Student Groups                 
##############################################################
...
ggsave("../plots/ACPS Middle Schools Career Self Efficacy Scores by Student Group.pdf")

##############################################################
# ACPS Self-Efficacy Subscores- by Ethnicity                          
##############################################################
...
mean_Eth <- career_eth %>% group_by(Ethnicity) %>% summarise(mean_score=mean(SE_subscore, na.rm=TRUE),
                                                                 obs=n())
eth.labs <- c("Asian Students", "Black Students", "Hispanic Students", "Multiracial Students", "White Students")
names(eth.labs) <- c("AS", "BL", "HL", "MR", "WH")

# Plot Creation
#### Plots ACPS self efficacy subscores by ethnicity with mean value and individual scores

ggplot(data=career_eth, aes(y=SE_subscore)) + 
  geom_point(aes(x=0, color=Ethnicity), position=position_jitter(0.1), alpha=.7) +
  geom_hline(data= mean_Eth, aes(yintercept = mean_score,col=Ethnicity), linewidth=1) +
...
ggsave("../plots/ACPS Middle Schools SE Subscores by Ethnicity.pdf")

##############################################################
# ACPS Self-Efficacy Subscores- by Student Group                        
##############################################################
...
mean_narm <- career_narm %>% summarise(mean_score=mean(SE_subscore, na.rm=TRUE),
                                             obs=n())
career_shp <- subset(career_narm, SHP=="SHP")
...
# Plot Creation
#### Plot of ACPS Self Efficacy subscores for student groups of interest 

ggplot() + 
  geom_point(data=career_narm, aes(y=SE_subscore, x=-1), position=position_jitter(0.1), alpha=.7, color="black") +
  ...
  labs(x="Student Group", y="Self-Efficacy Subscore", 
       title="ACPS Middle Schools Self Efficacy Subscores by Student Group")

ggsave("../plots/ACPS Middle SchoolS SE Subscores by Student Group.pdf")
...
mean_Eth <- career_eth %>% group_by(Ethnicity) %>% summarise(mean_score=mean(OE_subscore, na.rm=TRUE),
                                                                 obs=n())
eth.labs <- c("Asian Students", "Black Students", "Hispanic Students", "Multiracial Students", "White Students")
names(eth.labs) <- c("AS", "BL", "HL", "MR", "WH")
...
ggplot(data=career_eth, aes(y=OE_subscore)) + 
  geom_point(aes(x=0, color=Ethnicity), position=position_jitter(0.1), alpha=.7) +
  geom_hline(data= mean_Eth, aes(yintercept = mean_score,col=Ethnicity), linewidth=1) +
  labs(x="Ethnicity", y="Outcome Expectancy Subscore", 
       title="ACPS Middle Schools Outcome Expectancy Subscores by Ethnicity")+
...
ggsave("../plots/ACPS Middle Schools OE Subscores by Ethnicity.pdf")
...
mean_narm <- career_narm %>% summarise(mean_score=mean(OE_subscore, na.rm=TRUE),
                                             obs=n())
career_shp <- subset(career_narm, SHP=="SHP")
...
# Plot Creation
#### Plot of ACPS Outcome Expectancy subscores for student groups of interest 

ggplot() + 
  geom_point(data=career_narm, aes(y=OE_subscore, x=-1), position=position_jitter(0.1), alpha=.7, color="black") +
  ...
  labs(x="Student Group", y="Outcome Expectancy Subscore", 
       title="ACPS Middle Schools Outcome Expectancy Subscores by Student Group")

ggsave("../plots/ACPS Middle Schools OE Subscores by Student Group.pdf")
