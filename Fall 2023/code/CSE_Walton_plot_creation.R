##############################################################
# ACPS Career Self-Efficacy Analysis- Walton Pilot Analysis- Main Graphs and Appendices               
...
##############################################################
# Walton Middle School- Career Self Efficacy by Ethnicity               
##############################################################
...
mean_Eth <- career_walton %>% group_by(Ethnicity) %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                                                 obs=n())
...
# Plot Creation
#### Plots Walton's Career self efficacy scores by ethnicity with mean value and individual scores

ggplot(data=career_walton, aes(y=total_score)) + 
  geom_point(data=career_walton %>% dplyr::filter(Ethnicity=="WH"),
...
ggsave("../plots/Walton Middle School Career Self Efficacy Scores by Ethnicity.pdf")

##############################################################
# Walton Middle School- Career Self Efficacy by Student Groups                 
##############################################################
...
mean_walton <- career_walton %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                             obs=n())
career_shp <- subset(career_walton, SHP=="SHP")
mean_shp <- career_shp %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                     obs=n())
career_avid <- subset(career_walton, AVID=="AVID")
mean_avid <- career_avid %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                       obs=n())
career_el <- subset(career_walton, Current_EL=="EL")
mean_el <- career_el %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                   obs=n())
career_gg1 <- subset(career_walton, GG1=="GG1")
mean_gg1 <- career_gg1 %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                     obs=n())
career_sped <- subset(career_walton, SPED=="SPED")
mean_sped <- career_sped %>% summarise(mean_score=mean(total_score, na.rm=TRUE),
                                       obs=n())

# Plot Creation
#### Plot of Walton's Career Self Efficacy Scores for student groups of interest 

ggplot() + 
  geom_point(data=career_walton, aes(y=total_score, x=-1), position=position_jitter(0.1), alpha=.7, color="black") +
...
ggsave("../plots/Walton Middle School Career Self Efficacy Scores by Student Group.pdf")

##############################################################
# Walton Self-Efficacy Subscores- by Ethnicity                          
##############################################################
...
career_walton <- subset(career_narm, School=="Leslie H. Walton Middle School")
mean_walton <- career_walton %>% summarise(mean_score=mean(SE_subscore, na.rm=TRUE),
                                             obs=n())

mean_Eth <- career_walton %>% group_by(Ethnicity) %>% summarise(mean_score=mean(SE_subscore, na.rm=TRUE),
                                                                 obs=n())
eth.labs <- c("Asian Students", "Black Students", "Hispanic Students", "Multiracial Students", "White Students")
names(eth.labs) <- c("AS", "BL", "HL", "MR", "WH")

# Plot Creation
#### Plots Walton's self efficacy subscores by ethnicity with mean value and individual scores

ggplot(data=career_walton, aes(y=SE_subscore)) + 
  geom_point(data=career_walton %>% dplyr::filter(Ethnicity=="WH"),
             aes(x=0, color=Ethnicity), position=position_jitter(0.1), alpha=.7) +
  geom_hline(data= mean_Eth %>% dplyr::filter(Ethnicity=="WH"), aes(yintercept = mean_score,col=Ethnicity), linewidth=1) +
  labs(x="Ethnicity", y="Self-Efficacy Subscore", 
       title="Walton Middle School Self Efficacy Subscores by Ethnicity")+
...
ggsave("../plots/Walton Middle School SE Subscores by Ethnicity.pdf")

##############################################################
# Walton Self-Efficacy Subscores- by Student Group                        
##############################################################
...
# Plot Creation
#### Plot of Walton's  Self Efficacy subscores for student groups of interest 

ggplot() + 
  geom_point(data=career_walton, aes(y=SE_subscore, x=-1), position=position_jitter(0.1), alpha=.7, color="black") +
  ...
  labs(x="Student Group", y="Self-Efficacy Subscore", 
       title="Walton Middle School Self Efficacy Subscores by Student Group")

ggsave("../plots/Walton Middle School SE Subscores by Student Group.pdf")

##############################################################
# Walton Outcome Expectancy Subscores- by Ethnicity                        
##############################################################
...
career_walton <- subset(career_narm, School=="Leslie H. Walton Middle School")
mean_walton <- career_walton %>% summarise(mean_score=mean(OE_subscore, na.rm=TRUE),
                                             obs=n())

mean_Eth <- career_walton %>% group_by(Ethnicity) %>% summarise(mean_score=mean(OE_subscore, na.rm=TRUE),
                                                                 obs=n())
eth.labs <- c("Asian Students", "Black Students", "Hispanic Students", "Multiracial Students", "White Students")
names(eth.labs) <- c("AS", "BL", "HL", "MR", "WH")
...
ggplot(data=career_walton, aes(y=OE_subscore)) + 
  geom_point(data=career_walton %>% dplyr::filter(Ethnicity=="WH"),
             aes(x=0, color=Ethnicity), position=position_jitter(0.1), alpha=.7) +
  geom_hline(data= mean_Eth %>% dplyr::filter(Ethnicity=="WH"), aes(yintercept = mean_score,col=Ethnicity), linewidth=1) +
  labs(x="Ethnicity", y="Outcome Expectancy Subscore", 
       title="Walton Middle School Outcome Expectancy Subscores by Ethnicity")+
...
ggsave("../plots/Walton Middle School OE Subscores by Ethnicity.pdf")

##############################################################
# Walton Outcome Expectancy Subscores- by Student Group                        
##############################################################
...
# Plot Creation
#### Plot of Walton's Outcome Expectancy subscores for student groups of interest 

ggplot() + 
  geom_point(data=career_walton, aes(y=OE_subscore, x=-1), position=position_jitter(0.1), alpha=.7, color="black") +
  ...
  labs(x="Student Group", y="Outcome Expectancy Subscore", 
       title="Walton Middle School Outcome Expectancy Subscores by Student Group")

ggsave("../plots/Walton Middle School OE Subscores by Student.pdf")
